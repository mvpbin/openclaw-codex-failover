...
# ========= Config (override via env or /etc/openclaw-healthcheck.env) =========
...
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "[$(date -u +%H:%M:%S)] another healthcheck is running, skip" | tee -a "$LOG_FILE"
  exit 1
fi
...
if [[ -x "$BASE_DIR/scripts/sync_openclaw_auth_order.sh" ]]; then
  if "$BASE_DIR/scripts/sync_openclaw_auth_order.sh" "$PROVIDER" main >/dev/null 2>&1; then
